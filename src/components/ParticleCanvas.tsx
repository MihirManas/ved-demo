...
export default function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    class Particle {
      x: number; y: number; size: number; baseSpeedX: number; baseSpeedY: number; speedX: number; speedY: number; colorType: number; orbitRadius: number;
      ...
      update() {
        if (mouse.x != null && mouse.y != null) {
          ...
          if (distance < mouse.radius) {
            ...
            let radialForce = force * 0.35; 
            if (distance < this.orbitRadius) {
              radialForce = -0.8 * ((this.orbitRadius - distance) / this.orbitRadius);
            }

            this.speedX += forceDirectionX * radialForce;
            this.speedY += forceDirectionY * radialForce;

            this.speedX += tangentialX * force * 0.3; 
