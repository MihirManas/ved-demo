...
const FloatingActionButton = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 sm:bottom-10 sm:right-10 z-50 flex flex-col items-end">
      ...
      <div className={`flex flex-col gap-3 mb-4 transition-all duration-500 origin-bottom ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-50 translate-y-10 pointer-events-none'}`}>
        {menuItems.map((item, index) => (
          ...
          >
            ...
            <div className="w-12 h-12 rounded-full bg-white dark:bg-[#1F3145]/50 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-lg flex items-center justify-center text-gray-800 dark:text-white hover:text-[#E6C875] dark:hover:text-[#E6C875] hover:scale-110 hover:border-[#E6C875]/50 transition-all duration-300">
              {item.icon}
            </div>
          </a>
        ))}
      ...
      <button 
        onClick={toggleMenu}
        className="w-14 h-14 sm:w-16 sm:h-16 bg-[#1F3145] dark:bg-white/[0.05] backdrop-blur-2xl border border-gray-800 dark:border-white/10 hover:bg-gray-900 dark:hover:bg-[#E6C875]/10 hover:border-[#E6C875]/50 dark:hover:border-[#E6C875]/30 rounded-full flex items-center justify-center transition-all duration-500 ease-out hover:scale-110 shadow-2xl text-white group focus:outline-none"
      >
